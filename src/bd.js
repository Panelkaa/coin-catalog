...
app.get('/Search/:id', (req, res) => {
    connection.query(`SELECT * FROM coins.coin WHERE Title REGEXP '^${req.params.id}';`, (err, data) => {
        if (err) {
            console.error("Err: ", err);
            return res.status(500)
        } else {
            // const coinID = req.params.id;
            const result = data.filter((item) => item.Title === req.params.id)
            console.log(result)
            res.json(result);
        }
...
//    WHERE Title REGEXP '^${req.params.id}'
